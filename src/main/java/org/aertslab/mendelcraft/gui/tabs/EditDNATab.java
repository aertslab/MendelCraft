...
import org.aertslab.mendelcraft.gui.buttons.EditButton;
import org.aertslab.mendelcraft.items.ItemRegistry;
import org.aertslab.mendelcraft.network.LaboratoryPacket;
...
public class EditDNATab extends SyringeTab{
	...
	public void ChangeGene(LaboratoryScreen screen, int relX, int relY, Button b, String gene, int i, int parent) {
		...
		this.init(relX, relY);
		item.getCapability(DNAProvider.DNASTORAGE).ifPresent(cap -> {
			if (DNAUtil.getPossibleGenes(getSelectedChromosome(), getSelectedgene()) != null) {
