...
import org.aertslab.mendelcraft.config.ModConfig;
...
public class DNAUtil {
	...
	public static String getExtragene(Random random) {
		List<String> list = ModConfig.CONFIG.get().get("Effect.extra.gene");
	...
	private static String randomGenes(String chromosome, int gen, Level level){
		try {
			...
			if (list == null) {
				return getExtragene(level.random);
			}
			...
			e.printStackTrace();
			System.err.println(chromosome + " " + gen);
		}
		return "AAA";
	}
